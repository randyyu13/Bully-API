            ...
            while i < number_of_matchups:
                ...
                for prop in player_props:
                    ...
                    if player_name not in player_prop_set:
                        ...
                        player_prop_map = self.__populate_NBA_player_props(player_link, player_name, browser)
                        self.__upload_props_to_db(self, player_prop_map)
                i += 1
                matchups[i].click()
                page.wait_for_load_state("networkidle")
        
    def __populate_NBA_player_props(self, player_link, player_name, browser):
            ...
            for prop_type in player_props_map[player]:
                for sb in player_props_map[player][prop_type]:
                    ...
                    if number_of_cols == 4:
                        ...
                        odds = sb_data['odds']
                    self.__cloudsql_broker.write_to_player_prop_table(player, prop_type, sb, over_line, under_line, over_odds, under_odds, odds, datetime.now())